...
public class FetchPubTime {
	...
	public static String getPubTimeVarious(String url, String urlContent) {
		...
		if (urlContent != null && !urlContent.trim().equals("")) {
			ptime = extractPageDate(urlContent);
		}
		return ptime == null ? getPubTimeFromUrl(url) : ptime;
		/*
		 * String pubTime = getPubTimeFromUrl(url);
		 * 
		 * //链接里面没有，匹配文本中的 if (pubTime == null) { if (urlContent != null &&
		 * !urlContent.trim().equals("")) return extractPageDate(urlContent); }
		 * 
		 * return pubTime;
		 */
	}
	...
	public static String extractPageDate(String text) {
		...
		try {
			...
			Pattern p = Pattern.compile("(20\\d{2}[-/]\\d{1,2}[-/]\\d{1,2})|(20\\d{2}年\\d{1,2}月\\d{1,2}日)",
					Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
